...
function move2(animal: Fish | Bird | Person) {
    if('fly' in animal) {
...
function logValue(x: Date | string) {
    if(x instanceof Date) console.log(x.toDateString())
...
let x = Math.random() < 0.5 ? 1 : 'good joe'
x = 2
x = 'goodbye joe'
// x = true // type error!
...
// typescript 可以通过联合类型中每个类型独有的字面量来缩小类型范围
type Circle = {
    kind: 'circle',
    radius: number
}
...
function getArea(shape: Shape) {
    ...
    }else return Math.pow(shape.length , 2)
}
...
function getArea2(shape: Shape | Triangle) {
    let res = 0
